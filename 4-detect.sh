...
#SBATCH --output=/scratch/user/anthony.li/llm-watermark-adaptive/log/detect.%A.%a.out
#SBATCH --error=/scratch/user/anthony.li/llm-watermark-adaptive/log/detect.%A.%a.err
...
# Determine the total number of commands by counting lines in 4-detect-commands.sh
total_commands=$(wc -l < 4-detect-commands.sh)
total_jobs=1000
...
for i in $(seq $start_command $end_command); do
    command=$(sed -n "${i}p" 4-detect-commands.sh)
