...
def deletion_attack_semantic(tokens, tokenizer):
    ...
    attacked_tokens = tokenizer.encode(attacked_text,
                                       return_tensors='pt',
                                       truncation=True,
                                       max_length=2048)[0]
    ...
    while attack_start_addition < len(
            attacked_tokens_list) and attacked_tokens_list[
                attack_start +
                attack_start_addition] != original_tokens[attack_start]:
    ...
    for i in range(len(attacked_tokens_list) - attack_start_addition):
        if i >= len(original_tokens) or attacked_tokens_list[
                i + attack_start_addition] != original_tokens[i]:
            attack_start = i
            break
    else:
        return attacked_tokens, (len(attacked_tokens_list),
                                 len(original_tokens))

    found = False

    for i in range(attack_start + 1 + attack_start_addition,
                   len(attacked_tokens_list)):
...
def insertion_attack_semantic(tokens,
                              prompt,
                              tokenizer,
                              model,
    ...
    attacked_tokens = tokenizer.encode(attacked_text,
                                       return_tensors='pt',
                                       truncation=True,
                                       max_length=2048)[0].to(device)

    attack_start = len(
        tokenizer.encode(parts[0],
                         return_tensors='pt',
                         truncation=True,
                         max_length=2048)[0])
    attack_end = len(
        tokenizer.encode(parts[0].strip() + ". " + inserted_text,
                         return_tensors='pt',
                         truncation=True,
                         max_length=2048)[0])
