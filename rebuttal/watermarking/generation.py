    ...
    for i in range(m):
        ...
        if fixed_inputs is None:
            tokens, sampling_prob = sampler(
                probs, pis, xis[torch.arange(batch_size),
                                (offset.squeeze() + i) % n])
        else:
...
def generate_mixed(model,
    ...
    for i in range(m):
        ...
        if fixed_inputs is not None:
            ...
            # the results
            rows_no_watermark = torch.any(torch.isin(no_watermark_locations, i),
            ...
            messages=[
                {
                    "role":
                        "system",
                    "content":
                        "What might be the prompt used to generate the provided text? Start with the prompt directly."
                },
                {
                    "role": "user",
                    "content": text
