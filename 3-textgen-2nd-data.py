...
while itm < T:
    ...
    prompt = tokens
    prompt_max_length = max(prompt_max_length, len(prompt))
...
# Pad the prompts to the maximum length.
prompts = [
    torch.nn.functional.pad(
        prompt, (0, prompt_max_length - len(prompt)),
...
for batch in range(n_batches):
    ...
    null_samples.append(null_sample[:, prompt_max_length:])
    ...
    watermarked_samples.append(watermarked_sample[:, prompt_max_length:])
...
if not args.gpt_prompt_key:
    ...
    for batch in range(n_batches):
        ...
        null_sample, _, _ = generate_rnd(
            torch.vstack(icl_samples)[idx], icl_sample_max_length,
