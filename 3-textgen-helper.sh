...
for model_prefix in ml3; do
  ...
  for watermark_key_length in 10 20 30 40 50; do
    for attack in deletion insertion substitution; do
      ...
      for method in transform; do
        for pcts in "${pcts_list[@]}"; do
          echo "python 3-textgen.py --save results/$model_prefix-$method-$attack-$watermark_key_length-$tokens_count-$pcts --watermark_key_length $watermark_key_length --batch_size 25 --tokens_count $tokens_count --buffer_tokens 0 --model $model --seed 1 --T 100 --method $method --${attack} $pcts" >> 3-textgen-commands.sh
        done
      done
    done
  done
